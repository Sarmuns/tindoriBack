...
router.post("/login", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Erro ao validar o login do tutor", error: e });
  }
});
...
router.post("/cadastro", async (req, res) => {
  ...
  try {
    ...
    if (error) {
      return res
        .status(500)
        .json({ error: "Não foi possível cadastrar o tutor", error });
    }
    ...
    return res
      .status(201)
      .json({ message: "Tutor cadastrado com sucesso", tutor: data });
  } catch (e) {
    return res
      .status(500)
      .json({ error: "Erro ao cadastrar o tutor", error: e.message });
  }
});
...
router.get("/:id", async (req, res) => {
  ...
  try {
    ...
    return res
      .status(500)
      .json({ error: "Erro ao obter informações do tutor", error: e });
  }
});
...
router.patch("/:id", async (req, res) => {
  ...
  try {
    ...
    if (existingEmail.data) {
      return res
        .status(400)
        .json({ error: "Email já cadastrado para outro tutor" });
    }
  } catch (e) {
    return res.status(500).json({ error: e.message });
  }
  ...
  try {
    ...
    if (error) {
      return res
        .status(500)
        .json({ error: "Erro ao atualizar os dados do tutor" });
    }
    if (data != null) {
    ...
    return res
      .status(500)
      .json({ er: "Erro ao atualizar os dados do tutor", e: error });
  }
});
